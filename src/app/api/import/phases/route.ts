...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate first
    const validation = await DataImporter.validateImport(csvContent, 'phases');
    ...
    // Import the data
    const result = await DataImporter.importPhases(csvContent, user.id);
    ...
    return NextResponse.json({
      success: true,
      imported: result.imported.phases,
      failed: result.failed.phases,
