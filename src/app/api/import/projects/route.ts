...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate first
    const validation = await DataImporter.validateImport(csvContent, 'projects');
    ...
    // Import the data
    const result = await DataImporter.importProjects(csvContent, user.id);
